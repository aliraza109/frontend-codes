...
for (let i = 0; i < products.length; i++) {
	...
	manufacturerName.setAttribute('class', 'manufacturerName');
	manufacturerName.innerHTML =
		'Manufacturer : ' + products[i].specs.manufacturer;
...
function searchAndHighlightByManufacturer(manufacturer) {
	$('#products-list')
		.children()
		.each(function (i) {
			let productManufacturer = this.children[2].children[0].children[0].innerHTML.slice(
				15,
				25
			);
			if (
				$.trim(manufacturer.toLowerCase()) ===
				$.trim(productManufacturer.toLowerCase())
			) {
...
$(document).ready(function () {
	$('#manufacturer input,#manufacturer select').each(function (index) {
		if ($(this).is(':checked')) {
			alert('ok');
		}

		this.addEventListener('click', function () {
			searchAndHighlightByManufacturer(this.value);
		});
	});
	$('#manufacturer  input,#manufacturer  select').each(function (index) {
